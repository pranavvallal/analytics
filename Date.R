...
as.Date('2019-02-28') +1 

#default format %Y-%m-%d
dates2a = c('2018-04-21')
...
format(dates2b, format=('%d-%B-%y'))
format(dates2b, format=('%A-%B-%Y'))
...
format(as.date(21-04-2018,'%d-%m-%Y'),'%A')
...
course[c(1,6)] # 1st(not 0th) & 5th date 
course[1] ; course[11]
(duration = course[8] - course[1])
(duration2 = max(course) - min(course))
dob= as.Date('06-10-1994','%d-%m-%Y')
dob
dob1 = dob + 1:1000
dob1
Sys.Date()
...
boxplot((as.numeric(Sys.Date() - dob1)/365))
...
seq(as.Date("2016/1/1"), as.Date("2018/5/1"), "")
...
seq(as.Date("2017/1/1"), by = "quarter", along.with = c(3,2,43))  # 
...
(dateseqF = seq(as.Date("2017/1/7"), as.Date("2018/1/7"), by = "1 month"))
format(dateseqF, format='%A-%d-%B' )# order
