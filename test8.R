...
plot(y=df$Y, x=df$X,xlab='Area in sqft', ylab='Sales Amount', type='p', ylim=c(0, max(df$Y)), main='Plot of Area Vs Sales', xlim=c(0,max(df$X)), col='red',pch=9)

abline(lm(df$Y ~ df$X), lty=3, lwd=4, col='green') # with regression line
abline(v=c(3,5),h=c(6,10), col=c('green','blue')) # few straight lines at x & y axis
#Model
